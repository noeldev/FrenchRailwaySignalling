...
function p.Test()
	...
	-- Function to map target-types to states
    local function getStates()
		local states = {}
		local stateMap = info.stateMap
		local count = 0
	        ...
	        for char in mapEntry.pattern:gmatch(".") do
	            if char == 'x' then
	                ...
	                if not states[key] then
                        local state = mapEntry.states[1];
                        states[key] = state
                        mw.log(key .. "=" .. states[key])

                        -- Return states as soon as numTargets states are collected
                        count = count + 1
                        if count == numTargets then
                        	return states
                        end
        ...
        -- Return the collected states if numTargets is not reached
	    return states
	end
	...
	local signalList, plateList = enumSignalsAndPlates(states)
	mw.log("Dump:")
	for target, state in pairs(states) do
	    mw.log(target .. " = " .. state)
	end
